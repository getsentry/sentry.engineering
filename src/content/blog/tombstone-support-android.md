...
* the biggest issue always was size: since we must package binaries for `x86`, `x86_64`, `armeabi-v7a` and `arm64-v8a`, 
  ...
  sentry SDK code only accounts for 20% of that size; the rest is `libunwindstack` and C++ infra it depends on.
...
for users that run on Android 12+ which is a significantly growing portion of the incoming events and user-base (add 
...
replicating platform crash infrastructure that already exists on the device. Tombstones fix each of them:
...
* **All threads, fully symbolicated.** Where the `inproc` backend could only capture the crashing thread, tombstones 
  ...
  often just the victim of a problem that originated on another thread. Seeing all of them is the difference between
...
If you never used the Native SDK interfaces in your native code directly, you can evaluate your options of disabling
the NDK integration. If enough users of an app moved on to Android 12+ there is no further use in running both 
integrations.

If however the Native SDK interface is still in direct use, both integrations should work together without any visible
degradation in UX.
...
the session they emerge from, but scope can forensically be reconstructed) as though it were an `ANR`. With tombstones 
...
The deeper problem was that tombstones and the existing Sentry NDK integration (using `sentry-native-ndk`) report the 
same crash. The Native SDK catches the signal at runtime via its own signal handler and writes an envelope to the 
"outbox". The tombstone is generated by the platform's `debuggerd`, which is invoked after the Native SDK's signal 
handler chains to the previous handler, but the tombstone only arrives through `ApplicationExitInfo` on the next launch,
...
sending infrastructure during the crash. So, on next start, that infrastructure loads every envelope fully into memory, 
...
Native crash reporting interacts with session tracking in ways that required careful coordination. When the tombstone 
