...
![The stylish final look we're going to build for our product tour](../../assets/images/building-a-product-tour-in-react/look.png)
...
export const IssueDetailsTourContext = createContext<TourContextType<IssueDetailsTour> | null>(
  null,
);
```
...
```tsx
export function TourElement<T>({ children, TourContext, ...props }) {
...
export function TourElementContent<T>({ children, tourContextValue, title, description, stepId }) {
  ...
  return (
    <Fragment>
      <div className="cool-border" aria-expanded={isActive}>
        {children}
      </div>
      {isActive && (
        <FloatingTooltip>
...
return (
  <button onclick={dispatch({ type: "START_TOUR" })} disabled={!isRegistered}>
    Start
  </button>
);
```

![A demo of the final in-app product tour](../../assets/images/building-a-product-tour-in-react/demo.gif)
