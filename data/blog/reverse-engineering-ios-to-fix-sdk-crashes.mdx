...
images:
  [
    '/images/reverse-engineering-ios-to-fix-sdk-crashes/j89s1l8qg9fhznaf3hmgjsu9-screenshot.png',
    '/images/reverse-engineering-ios-to-fix-sdk-crashes/d1cmt81vg0pj3na1gjwg3pbp-image.png',
    '/images/reverse-engineering-ios-to-fix-sdk-crashes/x96pfqfnxaiebtv0ccey07g1-image-2.png',
  ]
layout: PostSimple
...
![Standard Printing Dialog on iPadOS](/images/reverse-engineering-ios-to-fix-sdk-crashes/j89s1l8qg9fhznaf3hmgjsu9-screenshot.png)

# The Beginning

On Nov 9th, 2025, [a new issue](https://github.com/getsentry/sentry-cocoa/issues/6725) was created in the Sentry Cocoa SDK repository, informing us that the Sentry SDK started to cause app crashes on iPadOS 26 when using the standard iOS [UIPrintInteractionController](https://developer.apple.com/documentation/uikit/uiprintinteractioncontroller?language=objc) while [Sentry's User Interaction Tracing](https://docs.sentry.io/platforms/apple/guides/ios/tracing/instrumentation/automatic-instrumentation/#user-interaction-tracing) is enabled:
...
Seeing the error message `unrecognized selector sent to instance` is always a bad sign, because it means that we try to access a property or call a method of an instance, which does not exist — something that should’ve been caught at compile-time already.
...
![Crash in Xcode](/images/reverse-engineering-ios-to-fix-sdk-crashes/d1cmt81vg0pj3na1gjwg3pbp-image.png)
...
At this point I started to ask in our Team Apple SDK Slack channel for additional input, so my colleague Itay started looking into it too.
...
![Recursive Description of a UISlider](/images/reverse-engineering-ios-to-fix-sdk-crashes/x96pfqfnxaiebtv0ccey07g1-image-2.png)

So Itay set up a [Runtime Header Analyzer / Browser](https://github.com/nst/RuntimeBrowser) to extract the actualy header of `UIPrintPanelViewController` on iOS 26. The output was similar, indicating it's still **not** a subclass of `UISplitViewController`, but at least a delegate of it, an [`UISplitViewControllerDelegate`](https://developer.apple.com/documentation/uikit/uisplitviewcontrollerdelegate):
...
**This is why swizzling is powerful but also considered risky.**

So how is this relevant for our case? Well, it helps us to figure out if the `isKindOfClass:` is really overridden in the `UIPrintPanelViewController` by looking at the implementation address in the lookup system in combination with the superclass hierarchy.
...
Great, it's the same address `0x0000000180097bc8`, indicating it's not overridden and proving assumption.
...
Another strong indicator that the implementation of this method is somehow performing special handling for `UISplitViewController` resulting in the invalid cast, crashing SDKs.
...
This might not be the prettiest code, but at least the SDK is not crashing anymore and we were able to quickly release a hotfix to handle this edge case.
