...
This already brings up the first opportunity where we decided against surfacing a concrete type, and instead implement it using a protocol, allow us to easily refactor otherwise public types in the future, reducing the amount of breaking changes.
...
The simplest solution would be changing the API to offer a `String` parameter to define the unit. But as these are standardized, we can use Swift's `enum` type to offer compile-time safety:
...
SentrySDK.metrics.counter(key: "warning", value: 2) // value explicitly set to 2 without a unit
