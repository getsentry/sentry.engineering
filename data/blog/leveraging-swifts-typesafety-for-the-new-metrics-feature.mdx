...
While the implementation of the first proof-of-concept was already done weeks ago, a lot of thought has been put into the design of the public API - the main point of interaction for our SDK users which must not be changed unless absolutely necessary.
...
We support homogeneous and heterogeneous arrays, including a fallback for unknown types handling compiler limitations.
All this should make collecting information with the new Metrics features straightforward and easy to understand, while staying extensible and forwards compatible.
