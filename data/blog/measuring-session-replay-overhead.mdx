...
With Session Replay, we have [two sampling modes](https://docs.sentry.io/platforms/javascript/session-replay/understanding-sessions/?original_referrer=https%3A%2F%2Fwww.google.com%2F#default-session-initialization) that differ in memory usage due to the amount of event buffering required. Entire session replays only buffer a few seconds worth of data as it is constantly uploading to Sentry. However, replays that record only on error need to buffer for longer periods as we want to give you enough context before the error happens, to make the replay useful. This buffer is reset on an interval so that it does not grow infinitely.
...
We packaged together the above ideas into a [package](https://github.com/getsentry/overhead-metrics) to measure performance overhead in any web application. Follow along as we show you how to use it.
...
Verify that everything is working by running the overhead tool on our demo scenario:

```bash
npx @sentry-internal/overhead -f demo.mjs
```
...
Example image is from the Playwright docs
...
In the Playwright Inspector window, change “Target” from Node.js “Test Runner” to “Library”
...
Re-run the overhead script but on your newly created scenario

```jsx
npx @sentry-internal/overhead -f myscenario.mjs
```
...
<aside>
💡 Should we actually post a real overhead scenario with Sentry + Replay and interpret the results for people?

</aside>

We hope this gives you the tools you need to answer questions about Sentry’s added overhead.
